				...
				<div class="panel-body">
					<ul class="nav nav-tabs">
						<?php
							...
							$sql = 'SELECT media_id FROM expansion_media WHERE expansion_id="' . $id . '"';
							...
							$content = '';
							$tabNames = ['Description', 'Rules', 'Media', 'Links'];
							for ($i = 1; $i <= count($tabNames); $i++)
							{
								...
								$content = $content . '"';
								$content = $content . '>';
								$content = $content . '<a href="http://localhost/games/expansion.php/?id=' . $id . '&tab=' . $i . '">';
					...
					<?php
						$sql = 'SELECT expansion_name, description FROM expansions WHERE expansion_id="' . $id . '"';
						...
						if($tab == 1)
						{
							...
							$content = $content . convertText($expansion_name);
